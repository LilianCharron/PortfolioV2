...
const Projects: React.FC = () => {
  ...
  return (
    <section
      ...
      className="py-16 relative overflow-hidden bg-[#0a1a2e]"
    >
      {/* Multi-layered lab lighting */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/20 to-transparent pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-5xl max-h-5xl bg-indigo-500/[0.04] blur-[150px] pointer-events-none" />
      <div className="absolute -top-1/4 -right-1/4 w-1/2 h-full bg-warning/[0.02] blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="absolute left-6 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-16 pointer-events-none z-10 opacity-30">
        <p className="rotate-90 text-[8px] font-black tracking-[0.5em] uppercase whitespace-nowrap">
          Core.Module.active
        </p>
        <div className="w-[1px] h-24 bg-theme mx-auto" />
        <p className="rotate-90 text-[8px] text-warning font-black tracking-[0.5em] uppercase whitespace-nowrap">
          Status: Operational
        </p>
      </div>
      <div className="absolute right-6 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-16 pointer-events-none z-10 opacity-30">
        ...
        <div className="w-[1px] h-24 bg-theme mx-auto" />
        <p className="-rotate-90 text-[8px] text-sky-400 font-black tracking-[0.5em] uppercase whitespace-nowrap">
          Deployments: 100%
        </p>
      </div>
      ...
      <motion.div
        style={{ y: y1, opacity }}
        className="absolute top-32 right-[10%] opacity-5 text-[7rem] font-black pointer-events-none hidden lg:block"
      >
        <FaCode />
      </motion.div>
      <motion.div
        style={{ y: y2, opacity }}
        className="absolute bottom-32 left-[10%] opacity-5 text-[7rem] font-black pointer-events-none hidden lg:block"
      >
        <FaTerminal />
      </motion.div>
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {projects.map((project, index) => (
            ...
            >
              <div className="relative group">
                ...
                >
                  <span className="italic opacity-50 text-[10px] absolute top-3 text-muted">
                    CASE
                  </span>
        ...
        >
          ...
          <p className="text-muted text-[11px] font-black tracking-[0.5em] uppercase max-w-sm mx-auto leading-relaxed">
            End of experimental section. Proceed to specialized documentation for technical
            deep-dives.
          </p>
        </motion.div>
      </div>
...
const TechStat = ({ icon, label, value, playSound }: any) => (
  <div
    onMouseEnter={() => playSound('hover')}
    className="flex flex-col items-center cursor-help"
  >
    <div className="text-warning/40 text-base mb-1.5">{icon}</div>
    <span className="text-muted text-[8px] font-black uppercase tracking-widest mb-0.5">
      {label}
    </span>
