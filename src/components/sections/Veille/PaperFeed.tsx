...
const PaperFeed: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBuilds = async () => {
            try {
                const response = await fetch('https://api.papermc.io/v2/projects/paper/versions/1.21/builds');
                ...
                >
                    ...
                    <div className="space-y-3 max-h-32 overflow-y-auto pr-2 custom-scrollbar">
                        {build.changes.length > 0 ? (
                            ...
                            <p className="text-xs text-muted/30 italic text-center py-4">No specific changes listed</p>
                        )}
                    </div>
