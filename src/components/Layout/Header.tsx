  ...
  return (
    ...
    >
      ...
      <nav className="container mx-auto px-6 flex justify-between items-center relative">
        <Link
          to="/"
          className="text-lg sm:text-xl font-black tracking-tighter group flex items-center gap-3"
        >
                ...
                link.subLinks ? (
                  <div key={link.name} className="space-y-4">
                    ...
                    >
                      {link.name}
                      <FaChevronDown
                        className={`text-xs transition-transform duration-300 ${isStagesOpen ? 'rotate-180' : ''}`}
                      />
                    </button>
                    <AnimatePresence>
                      {isStagesOpen && (
...
const NavDropdown = ({ label, subLinks, hoveredPath, setHoveredPath }: {
  label: string;
  subLinks: SubLink[];
  ...
  const isActive = subLinks.some((link) => location.pathname === link.path);
  ...
  return (
    ...
    >
      <button
        ...
        className={`flex items-center gap-2 px-4 py-1.5 rounded-lg transition-all font-black text-[10px] uppercase tracking-[0.2em] group relative ${isActive || hoveredPath === label ? 'text-warning' : 'text-muted hover:text-white'
          }`}
      >
        ...
        {(hoveredPath === label || (isActive && !hoveredPath)) && (
          <motion.div
            layoutId="nav-pill"
            className="absolute inset-0 bg-warning/10 rounded-lg pointer-events-none"
...
const NavLink = ({ to, children, hoveredPath, setHoveredPath }: {
  to: string;
  ...
  return (
    <RouterNavLink
      ...
      onClick={() => playSound('click')}
      className={({ isActive }) =>
        `px-4 py-1.5 rounded-lg transition-all font-black text-[10px] uppercase tracking-[0.2em] relative group ${isActive || hoveredPath === to ? 'text-warning' : 'text-muted hover:text-white'
        }`
      }
    >
      {({ isActive }) => (
