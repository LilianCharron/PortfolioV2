...
import { FaServer, FaNetworkWired, FaShieldAlt, FaChevronRight, FaPlus, FaMicrochip } from 'react-icons/fa';
...
const CompetencesPage: React.FC = () => {
    ...
    const keywords = ["OSI", "TCP/IP", "RGPD", "Gantt", "ITIL", "SLA", "VLAN", "VPN", "DMZ", "IDS/IPS", "Hyper-V", "Docker"];

    const sections: SkillCategory[] = [
        {
            category: "01 - Services & Support",
            icon: <FaServer />,
            color: "warning",
            glow: "#fbbf24",
            items: [
        ...
        >
            ...
            <div className="container mx-auto max-w-7xl relative z-10">
                <div className="flex flex-col items-center mb-24">
                    ...
                    >
                        <FaMicrochip className="text-sm animate-pulse" /> Expertise Core Modules
                    </motion.div>
                    ...
                    >
                        STACK <span className="text-transparent bg-clip-text bg-gradient-to-br from-warning via-orange-500 to-sky-500">&</span> SKILLS
                    </motion.h1>
                    ...
                    />
                    <p className="text-muted text-[8px] font-black tracking-[0.5em] uppercase">S.I.O Reference Framework Dashboard</p>
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-6 mb-24">
                    {sections.map((section, idx) => (
                        ...
                        >
                            <div className={`p-3 rounded-xl transition-all duration-500 ${activeTab === idx ? 'bg-warning text-bg scale-110 shadow-lg' : 'bg-alt text-muted group-hover:text-warning'}`}>
                                {section.icon}
                            </div>
...
const SkillBox = ({ title, content, color, glow }: { title: string, content: string[], color: string, glow: string }) => {
    // Determine the text color based on the dynamic color prop
    const textColorClass = color === 'warning' ? 'text-warning' : color === 'sky-400' ? 'text-sky-400' : 'text-green-500';
    const borderColorClass = color === 'warning' ? 'group-hover:border-warning/30' : color === 'sky-400' ? 'group-hover:border-sky-400/30' : 'group-hover:border-green-500/30';
    ...
    return (
        ...
        >
            ...
            <h3 className="text-xl font-black mb-8 flex items-center justify-between tracking-tight">
                {title}
                <div className={`p-2 rounded-lg border border-theme glass ${textColorClass} opacity-30 group-hover:opacity-100 transition-all shadow-inner`}>
            ...
            <ul className="space-y-4">
                {content.map((bullet, idx) => (
                    <li key={idx} className="flex gap-3 text-muted group-hover:text-inherit transition-colors">
                        <FaChevronRight className={`text-[10px] mt-1 ${textColorClass} opacity-40 group-hover:opacity-100 group-hover:translate-x-1 transition-all`} />
