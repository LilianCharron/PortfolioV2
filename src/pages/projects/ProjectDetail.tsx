    ...
    if (!project) {
        return (
            <div className="pt-32 text-center min-h-screen bg-page">
                <h2 className="text-4xl font-black uppercase tracking-tighter mb-8">ENTRY NOT FOUND</h2>
                <Link to="/" className="text-warning font-black tracking-widest hover:underline uppercase">Back to Terminal</Link>
            </div>
        );
    }
        ...
        >
            ...
            <div className="container mx-auto max-w-6xl relative z-10">
                <Link to="/" className="inline-flex items-center gap-4 text-muted hover:text-warning mb-12 transition-all group uppercase font-black tracking-widest text-xs">
                    <FaChevronLeft className="group-hover:-translate-x-2 transition-transform" /> [ EXIT_VIEWPORT ]
                </Link>
                ...
                >
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-6">
                            <span className="px-4 py-1.5 bg-warning/10 text-warning text-[10px] font-black uppercase rounded-full tracking-widest border border-warning/20 shadow-lg">CASE_FILE: {id?.toUpperCase()}</span>
                            <div className="h-[1px] flex-grow bg-theme" />
                            <span className="text-muted text-[10px] font-black tracking-[0.4em] uppercase">Status: Operational</span>
                        </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="p-8 glass border border-theme rounded-[2rem] space-y-6">
                                <h3 className="text-lg font-black flex items-center gap-4 uppercase italic">
                                    <FaMicrochip className="text-warning" /> Stack Info
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                    {project.technologies.map(tech => (
                                        <span key={tech} className="px-3 py-1 glass text-muted text-[9px] font-black uppercase tracking-widest rounded-lg border border-theme">
                                            {tech}
                                        </span>
                                    ))}
                                </div>
                            ...
                            {/* System Meta */}
                            <div className="p-10 border border-theme rounded-[2.5rem] space-y-4 opacity-30">
                                <div className="flex justify-between text-[8px] font-black uppercase tracking-widest">
                                    <span>HASH_ID</span>
                                    <span>{Math.random().toString(16).substring(2, 10).toUpperCase()}</span>
                                </div>
                                <div className="h-[1px] bg-theme w-full" />
                                <div className="flex justify-between text-[8px] font-black uppercase tracking-widest">
                                    <span>SECURITY_CLEARENCE</span>
                                    <span>LEVEL_4</span>
                                </div>
                            </div>
                        </div>
